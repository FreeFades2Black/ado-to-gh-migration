...
def scan_and_sanitize_repo(repo_path):
    ...
    if large_files:
        ...
        for f, sz in large_files:
            print(f"   [LFS] {f} ({sz:.2f} MB) -> Requires Git LFS")
    else:
    ...
    for root, _, files in os.walk("."):
        ...
        for file in files:
            ...
            if ext in BLACKLISTED_EXTENSIONS or (file in BLACKLISTED_FILES and root != "."):
                target_file = os.path.join(root, file)
                print(f"   [-] Purging forbidden sensitive file: {target_file}")
                try:
